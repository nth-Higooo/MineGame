...
const redo = () => {
    ...
    if(autoOpenLoss) {
        ...
        for(let i = latestStep.length-1; i>=0;i--) {
            ...
            squares[row*gridWidth+col].innerHTML = ''; //clear numbers
            squares[row* gridWidth+col].style.background = "#ffffff" // return to the white
            mines[row][col].discovered = false; //not open yet
            nMinesDiscovered--;
        }
    }
...
const setMines = () => {
    ...
    while (minesToPopulate > 0) {
        ...
        //squares[gridWidth * i + j].innerHTML = 'B' // to present the Bomb as B
    ...
    for (let i = 0; i < mines.length; i++) {
        for (let j = 0; j < mines[i].length; j++) {
            if (!mines[i][j].mine) {
                ...
                //squares[gridWidth * i +j].innerHTML = n //to present the numbers
            }
        }
    }
}
